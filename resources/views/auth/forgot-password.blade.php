...
<head>
    ...
    <title>Forgot Password - {{ $settings['site_name'] ?? 'Tindablog' }}</title>
    <!-- Fonts -->
    ...
    <style>
        ...
        .auth-card { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 24px; padding: 3rem; width: 100%; max-width: 480px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); }
        .auth-title { color: white; font-weight: 700; font-size: 2rem; margin-bottom: 0.5rem; text-align: center; }
        .auth-subtitle { color: #9ca3af; text-align: center; margin-bottom: 2.5rem; line-height: 1.6; }
...
<body>
    ...
    <div class="auth-card">
        <div class="text-center mb-4">
            ...
            <p class="auth-subtitle">Forgot your password? No problem. Just let us know your email address and we will email you a password reset link.</p>
        </div>

        <!-- Session Status -->
        @if (session('status'))
            <div class="alert alert-success bg-transparent border-success text-success text-center rounded-3 p-3 mb-4" style="border-width: 1px;">
        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div class="mb-4 pb-2">
                <div class="input-group">
                    ...
                    <input id="email" type="email" class="form-control" name="email" value="{{ old('email') }}" placeholder="Enter your email address" required autofocus>
                </div>
                @error('email')
                    <div class="mt-2 text-danger small text-center">{{ $message }}</div>
                @enderror
            </div>

            <button class="btn btn-primary w-100 mb-4" type="submit"><i class="fa-regular fa-paper-plane me-2"></i> Send Reset Link</button>
            
            <div class="text-center mt-3">
                <a class="auth-link" href="{{ route('login') }}"><i class="fa-solid fa-arrow-left me-1"></i> Back to Login</a>
            </div>
        </form>
    </div>
</body>
